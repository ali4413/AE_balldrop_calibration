...
%for ch_idx = 16
    ...
    burnIn = 6e5;
    matfile = ['results_mat_8/MCMC_4para_scale_sigma_sensor_' num2str(ch_idx) '_k_1000000_PpickAIC1bc_8.mat'];
    %wfmfile = ['results_mat_8/wfm_' num2str(ch_idx) '.mat'];
    load(matfile);
    %load(wfmfile);
...
%     h1 = figure;
%     h1name = ['results_fig_6/wfm_' num2str(ch_idx) '_notext.eps'];
%     tmicron = 1e6*(0:dt:5e3*dt);
%     X1=[tmicron,(fliplr(tmicron))];                %#create continuous x value array for plotting
%     Y1=[wfm_bot(1:LWin)',fliplr(wfm_top(1:LWin)')];              %#create y values for out and then back
%     h1bc=fill(X1,Y1,[0, 0.4470, 0.7410]);
%     set(h1bc,'facealpha',.5,'EdgeColor','none')
%     hold on
...
%     Y2=[(offset1+wfm_bot(LWin+1:LWin*2))',fliplr((offset1+wfm_top(LWin+1:LWin*2))')];              %#create y values for out and then back
%     h2bc=fill(X2,Y2,[0, 0.4470, 0.7410]);
%     set(h2bc,'facealpha',.5,'EdgeColor','none')
%     hold on
...
%     Y3=[(offset2+wfm_bot(LWin*2+1:LWin*3))',fliplr((offset2+wfm_top(LWin*2+1:LWin*3))')];              %#create y values for out and then back
%     h3bc=fill(X3,Y3,[0, 0.4470, 0.7410]);
%     set(h3bc,'facealpha',.5,'EdgeColor','none')
%     hold on
