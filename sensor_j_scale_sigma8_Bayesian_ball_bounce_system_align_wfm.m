...
sdt=zeros(Nm,1); % hyperparameter sigmadt
Ndt = 15*2; % 15 working sensors, 2 bouncing time interval
v0(1) = 1.311;  % v0 in [1.2 1.8] m/s
a(1) = 0.6115; % a in [0.4 1]
...
%%
%dim=4; %parameter space
%nlevDT = 0.0001; % time interval std
%Sigmadt_diag= (DT_obs*nlevDT).^2;
%nlev = 0.2; %
%Sigmadt_diag= (1e-4).^2*ones(Ndt,1);
%al=1/2; be=al/1e8; % assume E(1/sigmadt) = 1E8
%A = al+Ndt/2; % a+n/2
%B = be+sum((DT_obs-DT_theo).^2)/2; % b+SS/2
%Sigmadt=1/gamrnd(A,1/B,1);
Sigmadt=normrnd(1e-10,1e-11);
sdt(1)=Sigmadt;
LcurrHyperPrior = log(normpdf(1e-10,1e-11));
Sigmadt_diag= Sigmadt*ones(Ndt,1);
...
prop_Var_4=[(5e-5)^2 (2e-5)^2 (.1e3)^2 (.1e3)^2]; %
...
LcurrHyperPrior_all = zeros(Nm,1);
LcurrPrior_all = zeros(Nm,1);
LcurrLikely_all = zeros(Nm,1);
LcurrHyperPrior_all(1)=LcurrHyperPrior;
LcurrPrior_all(1)=LcurrPrior;
LcurrLikely_all(1)=LcurrLikely;
for k = 2:Nm   
    tmp = [v0(k-1); a(k-1); omega_s(k-1);varepsilon(k-1)] + (randn(1,4)*R_4)';
    while tmp(1)<1.0 || tmp(1)>1.5 || tmp(2)<0.5 || tmp(2)>0.9||tmp(3)<100e3 || tmp(3)>500e3 || tmp(4)<10e3 || tmp(4)>50e3
        tmp = [v0(k-1); a(k-1); omega_s(k-1);varepsilon(k-1)] + (randn(1,4)*R_4)';
    end
    ...
    % propose Sigmadt by inverse-gamma
    %B = be+sum((DT_obs-DT_theo_prop).^2)/2; % b+SS/2
    %Sigmadt_prop=1/gamrnd(A,1/B,1);
    %LPropHyperPrior = log(gampdf(1/Sigmadt_prop,A,1/B));
    % propose Sigmadt by normpdf
    Sigmadt_prop=normrnd(1e-10,1e-11);
    LPropHyperPrior = log(normpdf(Sigmadt_prop,1e-10,1e-11));
    Sigmadt_diag= Sigmadt_prop*ones(Ndt,1);
    ...
    alpha = min(1,exp(LPropHyperPrior + LPropLikely + LPropPrior - LcurrHyperPrior - LcurrLikely - LcurrPrior));
    if (rand()<alpha)
        v0(k)=v0_prop;
        a(k)=a_prop;
        ...
        sdt(k)=Sigmadt_prop;
        LcurrHyperPrior = LPropHyperPrior;
    ...
    LcurrHyperPrior_all(k)=LcurrHyperPrior;
    LcurrPrior_all(k)=LcurrPrior;
    LcurrLikely_all(k)=LcurrLikely;
    if k>1e5
    ...
    R_4=chol(V_4(:,:,k));
    if mod(k,1e5)==0
        save(['MCMC_4para_scale_sigma_sensor_' num2str(j) '_k_' num2str(k) '_PpickAIC1bc_8.mat'],'v0','a','omega_s','varepsilon','sdt')
    end
end
%save(['MCMC_4para_scale_sigma_sensor_' num2str(j) '_PpickAIC1bc_8.mat'],'v0','a','omega_s','varepsilon','sdt')
toc