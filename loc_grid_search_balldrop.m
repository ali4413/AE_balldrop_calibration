...
for G=1:2
    ...
    for ev=1:length(tbs)
        ...
        minerror=1;
        %tst_temp=tst(:,ev);
        tpt_temp=tpt{G,ev}; % 1 M sampling rate
        for loc_idx=1:num_grids
            if mod(G,2)==0
                ...
                t0(loc_idx)=mean(tpt_temp(tpt_temp>0)-travel_time_grids_P(tpt_temp>0,loc_idx)*1e-3);
                error2_t0(loc_idx)=norm(travel_time_grids_P(tpt_temp>0,loc_idx)*1e-3+t0(loc_idx)-tpt_temp(tpt_temp>0));
                error2(loc_idx)=norm(travel_time_grids_P(tpt_temp>0,loc_idx)*1e-3-tpt_temp(tpt_temp>0));
...
for G=1:2
    ...
    for ev=1
        ...
        mArrow3([0 0 Length],loc_grids(:,good_loc_idx{G}(ev))', 'linewidth',2, 'color', 'black')
        mis_loc(G,:)=abs([0-loc_grids(1,good_loc_idx{G}(ev)) 0-loc_grids(2,good_loc_idx{G}(ev)) Length-loc_grids(3,good_loc_idx{G}(ev))]);
    end
end
hold on
r1=Diameter/2; %inch
...
zlim([-1 80])
%%
figure;tpt_temp=tpt{1}; plot(tpt_temp(tpt_temp>0),'o'); hold on
plot(travel_time_grids_P(tpt_temp>0,good_loc_idx{1})*1e-3+t0_all{1},'ro')
figure;tpt_temp=tpt{2}; plot(tpt_temp(tpt_temp>0),'o'); hold on
plot(travel_time_grids_P(tpt_temp>0,good_loc_idx{2})*1e-3+t0_all{2},'ro')