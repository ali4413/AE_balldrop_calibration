...
prop_Var_4=[(7e-3)^2 (2e-3)^2 (0.4e3)^2 (0.3e3)^2]; %
...
for k = 2:Nm
    ...
    while tmp(1)<1.2 || tmp(1)>1.8 || tmp(2)<0.4 || tmp(2)>1||tmp(3)<100e3 || tmp(3)>1000e3 || tmp(4)<10e3 || tmp(4)>500e3
        tmp = [v0(k-1); a(k-1); omega_s(k-1);varepsilon(k-1)] + (randn(1,4)*R_4)';
    end
...
%save(['MCMC_4para_scale_sigma_sensor_' num2str(j) '_nlev' num2str(nlev) '_PpickAIC1bc.mat'],'v0','a','omega_s','varepsilon')


%post_mean_wfm = mean(d_sample_all,2);
% %% plot results
% figure;
% tmicron = 1e6*(0:dt:5e3*dt);
% subplot(3,1,1)
% plot(tmicron,d_obs_norm(1:LWin),'r','linewidth',1); hold on
% plot(tmicron,d_syn_norm_prop(1:LWin),'b','linewidth',1);
% %ylim([-.08 .08])
...
% plot(tmicron,d_syn_norm_prop(LWin+1:LWin*2),'b','linewidth',1)
% %ylim([-.08 .08])
% set(gca,'fontsize',14,'xticklabel',[])
% title('2nd Bounce')
% subplot(3,1,3)
% plot(tmicron,d_obs_norm(LWin*2+1:LWin*3),'r','linewidth',1); hold on
% plot(tmicron,d_syn_norm_prop(LWin*2+1:LWin*3),'b','linewidth',1)
% %ylim([-.08 .08])
% set(gca,'fontsize',14)
% title('3rd Bounce')
% xlabel('Time (\muS)')
%%
figure;
subplot(4,1,1); plot(v0(burnIn+1:k-1));
set(gca,'fontsize',14,'xticklabel',[])
title('Initial velocity')
ylabel('v_0 (m/s)')
subplot(4,1,2); plot(a(burnIn+1:k-1));
set(gca,'fontsize',14,'xticklabel',[])
title('Rebound coefficient')
ylabel('a')
subplot(4,1,3); plot(omega_s(burnIn+1:k-1)/1e3);
set(gca,'fontsize',14,'xticklabel',[])
title('Resonance frequency')
ylabel('\omega_s (kHz)')
subplot(4,1,4); plot(varepsilon(burnIn+1:k-1)/1e3);
set(gca,'fontsize',14)
...
%%
figure;
[H,AX,BigAx,P,PAx]=plotmatrix([v0(burnIn+1:k-1) a(burnIn+1:k-1) omega_s(burnIn+1:k-1)/1e3 varepsilon(burnIn+1:k-1)/1e3]);
